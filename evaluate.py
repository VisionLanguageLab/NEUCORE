...
def validate(model, args, vocab, output_type="metrics", max_retrieve = 50, split='val'):
    ...
    if output_type=="metrics":
        ...
        if args.data_name == "cirr":
            ...
            rk, rsubK, message, val_mes = deal_with_CIRR(args, vocab, results[0], split) # [0] because no category in CIRR 
            return rk, rsubK, message, val_mes
        else:
...
def compute_and_process_compatibility_scores(data_loader_query, data_loader_target,
    ...
    all_token_embs, all_fg_embs, all_bg_embs = compute_necessary_embeddings_img(data_loader_target, model, args)
    ...
    for data in tqdm(data_loader_query):
        ...
        _, txt, txt_len, img_src_ids, img_trg_ids, _, indices, nouns, nouns_lengths, mil_labels = data
        
        if torch.cuda.is_available():
            txt, txt_len = txt.cuda(), txt_len.cuda()
            nouns_lengths = nouns_lengths.cuda()
        ...
        txt_embs, word_feats = model.get_txt_embedding(txt, txt_len)
        ...
        for i, index in enumerate(indices):
            ...
            token_emb = all_token_embs[img_src_id]
            fg_emb = all_fg_embs[img_src_id]
            bg_emb = all_bg_embs[img_src_id]
...
def compute_necessary_embeddings_img(data_loader_target, model, args):
    ...
    for data in tqdm(data_loader_target):
        ...
        token_feat, fg_feat, bg_feat = model.inference_fg_bg(img_trg)

        # Initialize the output embeddings if not done already
        if all_fg_feats is None:
            emb_sz = [len(data_loader_target.dataset), args.embed_dim]
            all_token_feats = torch.zeros((emb_sz[0], token_feat.size(1) ,emb_sz[1]), dtype=token_feat.dtype, requires_grad=False)
            all_fg_feats = torch.zeros(emb_sz, dtype=fg_feat.dtype, requires_grad=False)
            all_bg_feats = torch.zeros(emb_sz, dtype=bg_feat.dtype, requires_grad=False)
            if torch.cuda.is_available():
                all_token_feats = all_token_feats.cuda()
                all_fg_feats = all_fg_feats.cuda()
                all_bg_feats = all_bg_feats.cuda()

        # Preserve the embeddings by copying them
        if torch.cuda.is_available():
            all_token_feats[indices] = token_feat
            all_fg_feats[indices] = fg_feat
            all_bg_feats[indices] = bg_feat
        else :
            all_token_feats[indices] = token_feat.cpu()
            all_fg_feats[indices] = fg_feat.cpu()
            all_bg_feats[indices] = bg_feat.cpu()

    return all_token_feats, all_fg_feats, all_bg_feats
...
if __name__ == '__main__':
    ...
    start = time.time()
    with torch.no_grad():
        rK, rsubK, message, val_mes = validate(model, args, vocab, split="val")
    print(message)
