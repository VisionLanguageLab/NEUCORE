...
from config import GLOVE_DIR, TORCH_HOME
...
class PatchEncoderImage(nn.Module):
    def __init__(self, opt):
        super(PatchEncoderImage, self).__init__()
        if opt.cnn_type == "resnet50":
            model = torchvision.models.resnet50(pretrained=True)
        elif opt.cnn_type == "resnet18":
            model = torchvision.models.resnet18(pretrained=True)
        else:
            raise ValueError(f"Cannot find model name {opt.cnn_type}!")
        self.model = model
        self.proj = nn.Conv2d(self.model.fc.in_features, opt.embed_dim, kernel_size=1)
...
class EncoderImage(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.proj = nn.Conv2d(opt.embed_dim, embed_dim, kernel_size=1)
    ...
    def forward(self, images):
        ...
        x = self.proj(out_7x7.squeeze(dim=1))
        x = rearrange(x, "B D H W -> B (H W) D")
        return x
...
class EncoderText(nn.Module):
    ...
    def forward_bigru(self, x, lengths):
        ...
        word_out, rnn_out = self.sent_enc(packed)
        # reshape output to (batch_size, hidden_size)
        rnn_out = rnn_out.permute(1, 0, 2).contiguous().view(-1, self.embed_dim)
        word_feats, max_ = nn.utils.rnn.pad_packed_sequence(word_out, batch_first=True)

        out = l2norm(rnn_out)
        return out, word_feats
