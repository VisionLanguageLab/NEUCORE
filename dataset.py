...
class MyDataset(data.Dataset):
    ...
    def load_image_feature(self, path):
        ...
        with open(path, "rb") as f:
            image = torch.from_numpy(pickle.load(f)) # shape eg. (self.size_of_loaded_feature)
        return image
    ...
    def get_transformed_nouns(self, nouns):
        #for noun in nouns:
        #    t = self.vocab(noun)
        #    if t in [56, 49, 19, 104, 34]:
        #        print(f"{t}: {noun}")
        #        import time
        #        time.sleep(1)
        return torch.Tensor([self.vocab(noun) for noun in nouns])
