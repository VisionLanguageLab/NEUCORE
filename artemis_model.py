...
class ARTEMIS(BaseModel):
    ...
    def __init__(self, word2idx, opt):
        ...
        self.noun_encoder = nn.Linear(300, 512)
    ...
    def compute_score_cpt(self, r_fg, r_bg, m, t_fg, t_bg, store_intermediary=False):
    ...
    def compute_score_broadcast_cpt(self, r, m, t):
        src_trg_fg, src_fg, src_bg, trg_fg, trg_bg = self.concept_net(r, t)
        cls_weights = self.noun_encoder(self.txt_enc.embed(self.all_nouns))
        cls_logits = src_trg_fg @ cls_weights.t()

        fg_score = self.compute_score_broadcast_FG(src_fg, m, trg_fg)
        bg_score = self.compute_score_broadcast_BG(src_bg, m, trg_bg)

        return fg_score + bg_score, cls_logits
