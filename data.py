...
def get_loader_single(opt, vocab, split, transform, what_elements="triplet",
    ...
    if opt.data_name == 'fashionIQ':
        ...
        dataset = CIRRDataset(opt.image_feature_dir, opt.annotation_dir, split, vocab, transform, what_elements, opt.load_image_feature)
...
def collate_fn_triplet(data):
    ...
    images_src, sentences, images_trg, raw_caps, dataset_ids, nouns, mil_labels = zip(*data)
    ...
    # Merge nouns (convert tuple of 1D tensor to 2D tensor)
    noun_lengths = torch.tensor([len(noun) for noun in nouns])
    nouns_padded = torch.zeros(len(nouns), max(noun_lengths)).long()
    for i, noun in enumerate(nouns):
        end = noun_lengths[i]
        nouns_padded[i, :end] = noun[:end]

    mil_labels = torch.stack(mil_labels, 0)

    return images_src, sentences_padded, lengths, images_trg, raw_caps, dataset_ids, nouns_padded, noun_lengths, mil_labels
...
def collate_fn_query(data):
    ...
    # Sort a data list by sentence length
    data.sort(key=lambda x: len(x[1]), reverse=True)
    images_src, sentences, img_src_ids, img_trg_ids, raw_caps, dataset_ids, nouns, mil_labels = zip(*data)
    ...
    return images_src, sentences_padded, lengths, img_src_ids, img_trg_ids, raw_caps, dataset_ids, nouns_padded, noun_lengths, mil_labels
