...
# -- experiment specific
PHASE=train # choose: train / eval / heatmaps / ptflops
MODEL=ARTEMIS # choose: ARTEMIS / TIRG / cross-modal / visual-search / late-fusion / EM-only / IS-only
DATASET="$1" # choose: fashionIQ / shoes / cirr / fashion200k
RUN=13121-all # if you want to train the same model several times, use this as a form of identifier
...
if [ "$DATASET" == "fashionIQ" ] ; then
    ...
    specific_args=(
        ...
        --cnn_type 'resnet152'
        --engine_depth 1
        --num_type_instruction 3
        --num_instruction_generator_fc_layer 1
        --num_parameter_generator_fc_layer 2
        --module_depth 1
    )
...
if [ "$PHASE" == "train" ] ; then
    ...
    python3 ${codepath}/train.py "${exp_args[@]}" "${specific_args[@]}" --num_epochs 8 --step_lr 4 --learn_temperature

    python3 ${codepath}/train.py "${exp_args[@]}" "${specific_args[@]}" --img_finetune --txt_finetune --learn_temperature --ckpt ${CKPT_DIR}/${exp_name}/ckpt.pth
